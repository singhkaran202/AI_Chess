...
import { getGeminiMove } from '../services/llmService';
...
function ChessGame({ gameMode, apiKey }) {
  ...
  const makeLLMMove = useCallback(async () => {
    ...
    try {
      ...
      // Call LLM API to get a move
      const llmMove = await getGeminiMove(
        apiKey, 
        game.fen(), 
        moveHistory,
        legalMoves
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '15px' }}>
            <strong>Mode:</strong> {gameMode === 'human' ? 'Human vs Human' : 'Human vs LLM'}
          </div>
